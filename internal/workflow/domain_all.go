...
func (d *domainAllFlow) Name() string {
	return "domainAll"
}

func (d *domainAllFlow) Description() string {
	return "domain -> subdomain -> portscan -> httpx -> vulscan alive url -> spider -> vulscan spider link"
}

func (d *domainAllFlow) Run(params *workflowParams) {
	// 子域名收集
	...
	if fileutil.CountLines(subFlow.finalOut) == 0 {
		return
	}
	// CDN 识别
	cdnFlow := new(cdncheckFlow)
	cdnFlow.Run(&workflowParams{
		target:    subFlow.finalOut,
		workSpace: params.workSpace,
		opt:       params.opt,
	})
